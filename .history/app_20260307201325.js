...
function snapshotLayers() {
  // toDataURL() with destination-out active captures blank — must snapshot as source-over
  return layers.map((l) => {
    const offscreen = document.createElement("canvas");
    offscreen.width = l.canvas.width;
    offscreen.height = l.canvas.height;
...
function saveToHistory() {
  ...
  if (historyStack.length > 20) {
    historyStack.shift();
    // historyIndex stays the same — it now points to the new last entry
  } else {
    historyIndex++;
  }
...
function applyState(state) {
  // Block shaving during async restore
  isDrawing = false;
  ...
  state.layers.forEach((data, i) => {
    ...
    img.onload = () => {
      ...
      if (loadedCount === layers.length) {
        // All layers done — safe to restore the rest of state
    ...
    // Set src after onload to avoid race condition on cached images
    img.src = data;
  });
}
