...
// 4. RAZOR CURSOR:
razor = this.add.image(0, 0, "razor_img").setDepth(100);

// Calculate the scale factor to make the razor 2 inches
const dpi = 96; // Typical screen DPI, adjust if necessary
const desiredSizeInPixels = 2 * dpi; // 2 inches in pixels
const originalWidth = razor.width; // Original width of the razor image
const scaleFactor = desiredSizeInPixels / originalWidth;

// Apply the scale factor to the razor
razor.setScale(scaleFactor);

this.input.setDefaultCursor("none");

// Make the razor follow the mouse pointer
